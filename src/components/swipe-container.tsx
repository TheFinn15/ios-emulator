...
export const SwipeContainer: React.FC<Params> = ({swipeTo, children, toBlur = true, ...etc}) => {
  const childrenRef = useRef<HTMLDivElement | null>(null);

  const [caught, setCaught] = useState(false);
  const [clickedPosition, setClickedPosition] = useState<SwipePosition>({x: 0, y: 0});
  const [swipedPosition, setSwipedPosition] = useState<SwipePosition>({x: 0, y: 0});
  const [percentSwipeComplete, setPercentSwipeComplete] = useState(0.0);

  document.onclick = (ev) => {
    if (childrenRef.current && !childrenRef.current?.contains(ev.target as Node)) {
      deactivateSwipe();
      return;
    }
  };
  ...
  function doSwipe({clientY, clientX, ...ev}: React.MouseEvent<HTMLDivElement>) {
    setSwipedPosition({x: clientX, y: clientY});

    if (caught && isCorrectPosToDirection()) {
      const result = {x: clickedPosition.x - swipedPosition.x, y: clickedPosition.y - swipedPosition.y};
      const completeSwipe = (isVerticalContainer ? result.y : result.x);
      const maxHeightScreen = (isVerticalContainer ? ev.pageY : ev.pageX);

      setPercentSwipeComplete(  parseFloat((completeSwipe * 100 / maxHeightScreen).toFixed(2)) );
  ...
  function activateSwipe({clientY, clientX}: React.MouseEvent<HTMLDivElement>) {
    setCaught(true);
  ...
  function deactivateSwipe() {
    ...
    if (etc.handler.onSwipe)
      etc.handler.onSwipe(swipeTo, initValue);
  }
  ...
  return (
      ...
      >
        ...
        {false &&  <div className="blurred-container">
